{{ csrf_field() }}
.form-group.row
  .col-sm-2
    %label.col-form-label{'for' => 'name'}
      Name
  .col
    %input#name.form-control{'type' => 'text', 'placeholder' => 'Enter Highlight name', 'required' => true, 'name' => 'name', 'value' => "#{ old('name', $highlight->name) }" }
.form-group.row
  .col-sm-2
    %label.col-form-label{'for' => 'lat'}
      Latitude
  .col
    %input#lat.form-control{'type' => 'text', 'placeholder' => 'Enter Highlight latitude', 'required' => true, 'name' => 'lat', 'value' => "#{ old('lat', $highlight->lat) }" }
.form-group.row
  .col-sm-2
    %label.col-form-label{'for' => 'lon'}
      Longitude
  .col
    %input#lon.form-control{'type' => 'text', 'placeholder' => 'Enter Highlight longitude', 'required' => true, 'name' => 'lon', 'value' => "#{ old('lon', $highlight->lon) }" }  
.form-group.row
  .col-sm-2
    %label.col-form-label{'for' => 'start_date'}
      Start Date
  .col-auto
    %input#start_date.form-control{'type' => 'date', 'required' => true, 'name' => 'start_date', 'value' => "#{ old('start_date', $highlight->start_date) }" }
.form-group.row
  .col-sm-2
    %label.col-form-label{'for' => 'end_date'}
      End Date
  .col-auto
    %input#end_date.form-control{'type' => 'date', 'required' => true, 'name' => 'end_date', 'value' => "#{ old('end_date', $highlight->end_date) }" }
.form-group.row
  .col
    %label.col-form-label{'for' => 'description'}
      Description
    %textarea#description.form-control{'name' => 'description' } = old('description', $highlight->description)
.form-group.row
  .col
    %label.col-form-label{'for' => 'attendees'}
      Attendees
    %select#attendees.form-control{'required' => true, 'name' => 'attendees[]', 'multiple' => true }
      - $prev = old('attendees', $highlight->attendees()->get()->pluck(['id'])->toArray())
      @foreach($attendees as $attendee)
      %option{'value' => $attendee->id, 'selected' => in_array($attendee->id, $prev)}
        {{ $attendee->first_name }} {{ $attendee->last_name }}
      @endforeach
